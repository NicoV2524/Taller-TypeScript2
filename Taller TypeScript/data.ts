...
let tseries:HTMLElement=document.getElementById("Series")!;

function mostrarSeries(series: Serie[]): void{
    let body: HTMLElement = document.createElement("tbody");
    for (let s of series) {
        let elemento: HTMLElement = document.createElement("tr");
        elemento.innerHTML = `
            <th colspan="1">${s.id}</td>
            <th colspan="3">${s.name}</td>
            <th colspan="2">${s.channel}</td>
            <th colspan="2">${s.seasons}</td>`;
        body.appendChild(elemento);
}
    tseries.appendChild(body); 
}
...
function promedio(series: Serie[]): void{
    ...
    const promedio=cantTemp/numSeries

    const resultadoElement = document.querySelector("#resultado-promedio");

    if (resultadoElement) {
        // Asigna el valor del promedio al elemento HTML
        resultadoElement.textContent = `Seasons Average: ${promedio.toFixed(0)}`;
    }
}
