...
var tseries = document.getElementById("Series");
function mostrarSeries(series) {
    var body = document.createElement("tbody");
    for (var _i = 0, series_1 = series; _i < series_1.length; _i++) {
        var s = series_1[_i];
        var elemento = document.createElement("tr");
        elemento.innerHTML = "\n            <th colspan=\"1\">".concat(s.id, "</td>\n            <th colspan=\"3\">").concat(s.name, "</td>\n            <th colspan=\"2\">").concat(s.channel, "</td>\n            <th colspan=\"2\">").concat(s.seasons, "</td>");
...
function promedio(series) {
    ...
    for (var _i = 0, series_2 = series; _i < series_2.length; _i++) {
        ...
        var temporadas = serie.seasons;
        numSeries += 1;
        cantTemp += temporadas;
    }
    console.log(numSeries);
    console.log(cantTemp);
    var promedio = cantTemp / numSeries;
    var resultadoElement = document.querySelector("#resultado-promedio");
    if (resultadoElement) {
        // Asigna el valor del promedio al elemento HTML
        resultadoElement.textContent = "Seasons Average: ".concat(promedio.toFixed(0));
    }
}
promedio(series);
